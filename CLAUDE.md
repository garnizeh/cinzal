...
**Blocking on `main` is a narrower set than "runs in CI", and the difference is checkable, not assumed.** Branch protection requires exactly three status checks — `check`, `secrets` and `bench-compare` — and there are no repository rulesets, so that is the whole enforcement surface. The `vuln` and `replay` jobs run on every pull request and block nothing: a red `vuln` merges today. Treat a red one as a stop anyway and say so, but do not describe it as enforced. Read the truth rather than this sentence when it matters: `rtk gh api repos/garnizeh/cinzal/branches/main/protection --jq .required_status_checks.contexts`.
