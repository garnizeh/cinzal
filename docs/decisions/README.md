...
| [D24](D24-opening-offer-guarantee.md) | D7 claims reachability alone guarantees a non-empty opening contract offer, but that assumes a 3-step Warehouse/Border floor GDD §6.1 constraint 6 does not actually provide (only 2, non-adjacency), and a Nobody's eligible tier set (Tier I alone) doesn't span D7's own "bands cover everything ≥3" argument — surfaced while implementing [#63](https://github.com/garnizeh/cinzal/issues/63), confirmed with a concrete counterexample | open |
