...
| [D36](D36-lease-rate-chokepoint-gate.md) | The lease-rate exit criterion can't be met by a `Config` edit — loosen `OperatorOptions`' chokepoint gate, or recalibrate GDD §22's 2–4/player band? — surfaced by [#204](https://github.com/garnizeh/cinzal/issues/204)'s own root-cause finding, filed as [#231](https://github.com/garnizeh/cinzal/issues/231) | **decided** — neither: a D35-rigor paired-cohort sweep against the gate (10,000 matches, 95% intervals), shipped default vs. fully open, shows the lease rate interval tops out at [0.279, 0.289]/player — still 7–14× below the band, and below even the `<1` failing floor — at a paired RP-margin interval [−0.242, −0.201] entirely on the losing side of zero, closing the loosening option outright and undercutting the recalibration option's premise that the gate alone explains Operator's low leasing; deferred to M5.5's human playtesting per the roadmap's own "a sweep narrows the range, M5.5 confirms it" caveat, with Operator's contract-first priority ordering — a candidate next constraint, not established here — filed separately as [#237](https://github.com/garnizeh/cinzal/issues/237), blocked on that human data |
