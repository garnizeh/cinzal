...
| [D36](D36-lease-rate-chokepoint-gate.md) | The lease-rate exit criterion can't be met by a `Config` edit — loosen `OperatorOptions`' chokepoint gate, or recalibrate GDD §22's 2–4/player band? — surfaced by [#204](https://github.com/garnizeh/cinzal/issues/204)'s own root-cause finding, filed as [#231](https://github.com/garnizeh/cinzal/issues/231) | **decided** — neither: a paired-cohort RP-margin sweep against the gate, from the shipped default down to fully open, shows the lease rate still tops out at 0.303/player (6–13× below the band) at a cost that flips Operator from beating Runner to losing to it, closing the loosening option outright and undercutting the recalibration option's premise that the gate is what's calibrating Operator's leasing frequency; deferred to M5.5's human playtesting per the roadmap's own "a sweep narrows the range, M5.5 confirms it" caveat, with the actual constraint (Operator's contract-first priority ordering) filed separately as [#237](https://github.com/garnizeh/cinzal/issues/237), blocked on that human data |
