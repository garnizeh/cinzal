...
The row stays, struck through, because the numbering is cited elsewhere and a silently vanished D15 reads as an oversight. **D3–D14 are the twelve decisions that block M1** — joined since by D23–D29, surfaced later in M1's own implementation rather than in the original roadmap pass; see the table below for the complete current list.
