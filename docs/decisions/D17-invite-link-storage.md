...
**The raw-token handoff at creation.** `invite_links` stores only `token_hash` — by design, per sub-question 3, the raw token is never persisted. That means the *only* moment the raw token exists anywhere outside a client's URL bar is inside the handler that generates it. `POST /matches` (create) is that handler: it draws the 32 bytes, computes and stores `token_hash`, and must return or render the resulting invite URL **in that same response** — there is no second chance, since `SELECT` on `invite_links` afterward yields only the hash. This is the same generate-once, show-once shape as any bearer credential minted server-side (an API key, a TOTP provisioning secret); it costs no new schema, only a requirement on what the creation handler's response carries.
...
- **M5's lobby/join flow** can now be built: `POST /matches` (create) mints the match's first `invite_links` row in the same transaction and returns the raw-token invite URL in that response — the only place it ever will (see Reasoning's "raw-token handoff"); the join handler's full check is `token_hash` found AND `revoked_at IS NULL` AND `(expires_at IS NULL OR expires_at > now())` AND `matches.status = 'lobby'` AND an open seat exists.
