...
- **Retention:** notes survive a match finishing — a player may want to revisit their own analysis — but are **never** part of the replay bundle. `{seed, config, players, orderLog}` (RFC §10.4/§15.4, the `players` field added by [D57](D57-replay-bundle-player-count-field.md)) is shared between every player in the match; `board_notes` is seat-private and has no place in a bundle built to be handed to an opponent.
