...
- **Reversible at low cost.** Like D27 and D31, a later product decision that "seen" should mean "board opened" rather than "order submitted" is a superseding decision that only replaces where this same `UPDATE` statement's trigger fires from — the column, its type, its default, and its derived-from-`orders` status all still hold unchanged.
