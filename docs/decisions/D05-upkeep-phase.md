...
The issue assembled a best-reading list from the rules that need a per-round decrement and have no other stated home, then named four things it does *not* settle: the order of steps within the phase, whether upkeep emits `Event`s, which steps [D11](D11-config-suppression-flags.md)'s subsystem-suppression flags silence, and whether round 15 runs upkeep before final scoring counts live leases.
...
- **Everything else is private** to the affected seat's own `SelfState`/recap history — the contract-deadline miss and its penalty, the next-round-modifier clears. None of them has a row in RFC §9.1's table, and `OpponentView` carries no contract information at all, so there is no channel through which any of these could reach another seat without adding a twelfth writer this decision doesn't authorise. (`Flagged`/`EvasiveStepPenalty` no longer belong to this list at all — see below.)
- **Sinkhole clearing isn't seat-scoped at all** — no player "owns" a Sinkhole, so its `Event` has no `SelfState`/recap home to land in either. "No announcement" means no trail/anchor entry, not no `Event`: the record exists (RFC §6.7's blanket rule still applies, for the debug trace and telemetry), it just has no player-facing channel to be delivered through, public or private. This decision fixes the visibility class per row — public anchor, private-to-seat, or player-facing-absent — the same level of detail every other row in RFC §9.1's table gets; the concrete `kind`/`params` shape (RFC §11.5) for each is an implementation detail for whoever writes `game.Event`, not something this or any prior decision in this repository pins down.

**Suppression ([D11](D11-config-suppression-flags.md), decided since).** No step above needs a branch. Each operates by iterating state that already exists — held leases, active contracts, active Sinkholes, active next-round modifiers. When `Config.Suppress` disables a subsystem, no such state is ever created (Stake Post and Deal are rejected by `Legal()`, the event/incident decks are never built), so the corresponding step iterates zero items and is a no-op as a consequence of empty input, not of a suppression check written into Upkeep. This matches D11's own framing, quoted in the roadmap: flags belong in `Config`, "not as branches in `rules`." Written here while D11 was still open, flagged as consistent with whichever way it resolved rather than resolved by it — the same posture [D9](D09-node-type-rounding.md) took toward placement feasibility — and D11's actual decision confirms the prediction: every guard it defines sits at `Legal()` or Setup's deck-build step, never inside Upkeep.
...
- **[D11](D11-config-suppression-flags.md) inherited no new obligation, and its actual resolution confirms it.** It only had to guarantee that suppressed subsystems never populate the state Upkeep iterates over — Upkeep itself needed no further change, and none was made: D11's guards live in `Legal()` and Setup's deck-build step.
