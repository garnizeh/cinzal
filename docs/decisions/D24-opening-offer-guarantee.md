...
**This is why E is not the Option C the issue rejected.** C proposed strengthening constraint **6** — node-type placement — so that *every* Warehouse D23's fog seeding can surface is guaranteed a Border in `[3,4]`. Two things separate it from E, and both come from which constraint it picked:

- **Placement cannot see the starts.** `attemptGenerate` runs `assignNodeTypes` before `selectStartPositions`, and it must: constraint 5 measures distances between starts, which do not exist yet. So a constraint-6 rule has no way to say "the Warehouses near a start" — the phrase has no referent at the time the rule is enforced. Expressed as a placement rule at all, C's requirement can only be the global one (*every* Warehouse has a Border in `[3,4]`), which is in tension with constraint 6's own non-adjacency clause pushing Warehouses and Borders apart, and is plausibly infeasible on a 12-node map with four sectors. Expressed at the point where "near a start" does mean something, it stops being a constraint-6 rule and becomes a constraint-7 one — E's mechanism.
- **All, versus at least one.** Even read at that point, C asks that *every* Warehouse within 2 steps of a start be contractable. E asks that *at least one* be. C's accepted starts are a strict subset of E's, so it can only reject more attempts for a guarantee that is no stronger — the opening offer needs one valid pair, not every possible one.

E constrains **which starts qualify**, and the generator already gets to choose those: `selectStartPositions` walks a shuffled order of every node looking for a qualifying set, so tightening what qualifies costs a longer walk, not a structural redesign. The issue's framing collapsed C and E into "a map-generation fix," and they are neither the same fix nor the same cost.
