...
- **No RNG accounting changes.** `PurposeStartSelect` still consumes exactly `n-1` draws (one full shuffle), and the predicate that filters the shuffled order draws nothing. RFC §6.4's table and GDD §21's inventory are both unaffected. What does change is the *attempt count* — a stricter constraint 5/7 rejects more attempts — but attempts were never fixed-cost, and `rand` is deliberately threaded across attempts without reset (#59), so this is the mechanism working as specified rather than a new cost to account for.
...
- **GDD §6.1 constraint 7, §8.1 and §8.2 are edited, with a changelog entry — this PR.** Constraint 7 gains the contractable-Warehouse requirement; §8.1's "Constraint 7 (§6) then guarantees a **Known** Warehouse to originate from" paragraph gains the destination half of the guarantee; §8.2's "can only happen if... unreachable" sentence is corrected as quoted above. The GDD is authoritative on rules, so the rule text moves here, not in the implementing PR.
