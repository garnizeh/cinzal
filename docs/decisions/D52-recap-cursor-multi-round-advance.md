...
2. **Their first `POST /m/{id}/order`**, which both re-seats them (ending Autopilot, §8.2) and is a first submission for that round, advances the cursor by exactly one round — not to `round − 1`. The next Recap view shows N − 1 rounds, not zero.
3. **Every subsequent distinct-round submission** advances the cursor by one further round, so the same accumulated backlog reappears, one round narrower each time, until the seat has been caught up for `N` consecutive rounds. No round generated during the Autopilot window is ever silently discarded by the act of returning.
...
- **RFC §7.2/§8.2's "derived, rebuildable" rebuild expression is unchanged.** `last_seen_round` is still reconstructible from `orders` alone as `COALESCE((SELECT MAX(round) FROM orders WHERE match_id = X AND seat = Y AND source = 'human') − 1, 0)` — this decision changes how the column advances during live play, not what it means or how `cmd/replay --rebuild` regenerates it from a full scan. A rebuild always sees the final state, not the round-by-round trajectory, so it is unaffected by whether increments happened one round at a time or in one jump.
