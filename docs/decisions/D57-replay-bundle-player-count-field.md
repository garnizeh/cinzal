...
- **Reversible at low cost if a future corruption case shows the check is wrong** (for example, a legitimate reason a finished match's order log could omit a seat's rows entirely, which no case in the current GDD/RFC describes): dropping `Players` back to pure derivation is a one-field, one-check removal with no data migration, since every currently-exported bundle would still decode identically under Option A (the field would just go unread).
