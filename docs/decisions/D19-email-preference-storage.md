...
**Status:** decided
**Blocks:** the M3 schema migration, and M6's outbox worker, which must consult the preference before it enqueues rather than before it sends.
**Decided:** 2026-08-26
...
- **RFC §19's security table** gains an "Email preferences / unsubscribe" row: token found by `(match_id, seat)` first, not a standalone lookup key, unlike invite links; SHA-256 at rest; `GET` never mutates; `POST` is the only write path.
