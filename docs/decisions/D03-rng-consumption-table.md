...
**[D11](D11-config-suppression-flags.md) makes both totals conditional.** Under `Config.Suppress.Events` / `.Incidents`, the corresponding deck is never built at all — zero draws, not 23 or 25 — because solo scenarios that suppress a subsystem also suppress the Headline line that would announce it (GDD §14.1). The four counts above are each this decision's number for the default, unsuppressed case.
...
- **The event/incident deck cost (23 + 25 = 48 draws) happens once, at Setup, before round 1.** It has no interaction with the lazy-draw rule — nothing *within a round's resolution* is conditional here — so it adds no new row to §6.4's early-termination table. **[D11](D11-config-suppression-flags.md) later made the cost itself conditional on `Config.Suppress`**, but that is a Setup-time flag check (build the deck or don't), not an early-termination branch inside `Resolve` — a different question from the one the early-termination table answers, so that table still needs no new row.
