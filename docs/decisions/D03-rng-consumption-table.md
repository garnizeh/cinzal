...
| Scaffolding — one sector | `event.scaffolding` | **1** | Phase 6. Candidates: the sectors (four, per current §6.1 — **pending D8**), sorted by `SectorID`. Pool size not yet provably non-empty — see Reasoning. |
...
| Sinkhole — one node in the unstable sector | `incident.sinkhole` | **1** | Phase 7. Candidates: nodes in the sector already flagged by `incident.sector`, sorted by `NodeID`. Pool size not yet provably non-empty — **pending D8**, see Reasoning. |
...
**Sinkhole (one node in the flagged sector) and Scaffolding (one sector, of "the four") cannot be proven safe yet, and this decision says so rather than assuming it.** Both pools are bounded by the same unresolved fact: §6.1 constraint 3 requires four sectors of 4 to 8 nodes each — a floor of 16 — and that is exactly what [D8](https://github.com/garnizeh/cinzal/issues/46) says is "arithmetically impossible" at 12, 15 and 16 nodes, which are respectively GDD §19.1's *First Run* scenario, the two-player map, and *The Trail* scenario. Until D8 picks one of its own three options (relax the per-sector minimum, reduce sector count on small maps, or raise the node count), this decision cannot say a flagged sector always has at least one node to offer Sinkhole, or that there are always exactly four sectors for Scaffolding to pick among — and it would be dishonest to claim otherwise just because the three type-share pools above check out. Both rows' *method* (partial Fisher-Yates over whatever the sector partition turns out to be) is unaffected by which way D8 goes; only the safety argument is pending.
...
- **Candidate-pool safety splits three ways, and only one part of it is settled by this decision.** Dragnet, Shipping Boom and Fence's Windfall are safe now, at every node count named anywhere (12 and up) — a total-node-count argument that never touches sectors, so it survives D8 unchanged and depends only on [D9](https://github.com/garnizeh/cinzal/issues/47) not adopting a degenerate rounding rule. Bridge Down is safe now too, on its own, unrelated argument (fires at most once, against a 21+-edge floor). **Sinkhole and Scaffolding are not yet provably safe — both pools are exactly the sector-size question [D8](https://github.com/garnizeh/cinzal/issues/46) is still deciding**, and this decision does not resolve it. Whoever implements #56 (RNG) or #59 (`rules/gen`) must re-check both rows once D8 lands, specifically for the smallest node count D8 ends up permitting.
