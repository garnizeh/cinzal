...
| Scaffolding — one sector | `event.scaffolding` | **1** | Phase 6. Candidates: the sectors (four, per current §6.1), sorted by `SectorID`. Safe at 20/22/25/28 nodes by construction; **pending D8** only at 12/15/16 — see Reasoning. |
...
| Sinkhole — one node in the unstable sector | `incident.sinkhole` | **1** | Phase 7. Candidates: nodes in the sector already flagged by `incident.sector`, sorted by `NodeID`. Safe at 20/22/25/28 nodes by construction; **pending D8** only at 12/15/16 — see Reasoning. |
...
**Dragnet (Borders), Shipping Boom (Warehouses) and Fence's Windfall (Black Markets) depend only on total node count, not on sector structure, and are safe today.** §6.2's shares (24% Border, 24% Warehouse, 20% Black Market) applied to **12 nodes — the smallest node count named anywhere in either document**, GDD §19.1's *First Run* scenario — give, even under the most pessimistic independent floor rounding for each type, 2 Borders, 2 Warehouses and 2 Black Markets: enough for Dragnet's 2 and Shipping Boom's/Fence's Windfall's 1. This holds at every larger node count the GDD or the roadmap names too, and it holds **regardless of how D8 resolves**, because it never touches the sector question at all — only D9's eventual rounding rule, which would have to be pathological (dropping a whole node type to zero) to break it.

**Sinkhole (one node in the flagged sector) and Scaffolding (one sector, of "the four") are safe at every node count *except* the three D8 already flags, and this decision draws that line precisely rather than hedging the whole table.** §6.1 constraint 3 — four sectors, each 4 to 8 nodes, internally connected — is enforced by rejection sampling: "the generator rejects and retries until all hold." That is not a soft target, it is a gate a graph cannot ship without clearing, which means **any graph the generator ever accepts already has a non-empty, in-range sector count by construction** — no separate argument is needed for Sinkhole's or Scaffolding's pool once a match exists at all. So the only real question is *whether generation can succeed in the first place*, and that fails exactly where four sectors of at least 4 don't fit the node count — a floor of 16. At the node counts currently named that clear that floor with room to spare (20, 22, 25 and 28), both rows are safe today, no dependency on anything. At the three that don't — **12 and 15 nodes outright, and 16 nodes on a zero-slack boundary** ([D8](https://github.com/garnizeh/cinzal/issues/46)'s own list: GDD §19.1's *First Run* and *The Trail* scenarios, and the two-player map) — generation cannot be shown to terminate at all under the current constraint, and that is D8's question, not this decision's to answer or to invent a fallback for. Both rows' *method* (partial Fisher-Yates over whatever sector partition generation produced) is unaffected by which of D8's three options wins; only those three node counts' safety is pending.
...
- **RFC §6.4's table changes by a net eleven rows.** Nine consumers that were entirely absent get added (Dragnet, Bridge Down, Festival, Scaffolding, Shipping Boom, Fence's Windfall, Sinkhole and Rotating borders — eight resolved — plus Riot, reserved), and the table's two existing `deck.*` rows are replaced by the four decomposed ones above (a net +2). 9 + 2 = 11. The prose immediately below the table — the Torn Map mandate and the lazy-draw section — needs no change; every row here follows the existing pattern rather than adding a new one.
- **Candidate-pool safety splits three ways, and only one part of it is settled by this decision.** Dragnet, Shipping Boom and Fence's Windfall are safe now, at every node count named anywhere (12 and up) — a total-node-count argument that never touches sectors, so it survives D8 unchanged and depends only on [D9](https://github.com/garnizeh/cinzal/issues/47) not adopting a degenerate rounding rule. Bridge Down is safe now too, on its own, unrelated argument (fires at most once, against a 21+-edge floor). **Sinkhole and Scaffolding are safe at 20, 22, 25 and 28 nodes today — constraint 3's rejection sampling guarantees it by construction — and unresolved only at 12, 15 and 16**, exactly [D8](https://github.com/garnizeh/cinzal/issues/46)'s own problem set. Whoever implements #56 (RNG) or #59 (`rules/gen`) needs no re-check at the larger sizes; the only open item is whichever of D8's three options ends up governing the small-map case.
- **Riot's row stays open until [D4](https://github.com/garnizeh/cinzal/issues/42) resolves.** `internal/rules`'s RNG table and its index-count test cannot be complete until then; the property test in RFC §16.2 should carry a `t.Skip` (or equivalent) on Riot specifically, not on the table as a whole, so the other twelve rows are enforced from the first commit rather than waiting on D4.
