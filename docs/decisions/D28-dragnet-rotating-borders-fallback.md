...
- Against: introduces a special case conditional on both Dragnet being this round's card and its 2 seals happening to hit the full active set (2 players only) — not rare when both hold (up to 2/3 of the time on an even round, per the probabilities above), but its overall frequency depends on how often Dragnet itself is drawn from the 24-card global deck. A reader has to notice the special case exists at all.
...
**Lowest-`NodeID` is an arbitrary but deterministic tiebreak, exactly like every other "pick one, deterministically, with no RNG draw" choice already in this codebase** (`bySeat` ordering, D03's own A/B assignment — unlike a Fisher-Yates draw, which is replay-*reproducible* but is not itself RNG-free) — it costs nothing in the consumption table and reproduces identically on replay. Nothing about *which* Border reopens is meaningful to the player; only that one does.
...
- **`internal/rules` gains one new pure function, `blockedBordersThisRound`**: it receives Dragnet's already-drawn sealed Borders (computed earlier the same round, ahead of Step 0 validation, exactly as today), computes rotation's inactive-set complement for the requested round and player count itself, and applies the reopen check before delivery validation runs. No RNG consumption changes; RFC §6.4's table needs no new row.
