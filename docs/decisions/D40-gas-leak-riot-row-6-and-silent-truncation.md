...
1. **Does row 6 get corrected to count them?** At the shipped default, Operator, pooled, row 6 reads 20.61% / 26.36% / 31.73% / 35.93% at 2/3/4/5 players against a `< 20%` failing line — under D35's own verdict rule, the 2-player *interval* is `[20.47%, 20.74%]`, clearing the line by only 0.47 points at its lower bound (0.61 on the point estimate alone), and two more hit-capable cards moves the aggregate by materially more than that.
...
- §22 row 6's measured numbers are expected to change at every player count once the fix lands — not yet measured, and not a claim this document makes about the resulting values. [#205](https://github.com/garnizeh/cinzal/issues/205)'s exit-demo doc needs its R6 section re-run and re-read against the corrected stream once [#274](https://github.com/garnizeh/cinzal/issues/274) lands — not re-litigated, since R6's verdict already stands as NO ACTION and already names this under-count as the reason to expect the numbers to move.
