...
**One slot is deterministic in its *target*.** Of the three offered contracts, the first always **targets** the player's **single highest currently eligible tier**, using their Infamy at Phase 2 (GDD §11.1 — cited, not re-decided here). No draw decides *which* tier this slot targets; it is a pure function of Infamy. The guarantee is on the target, not unconditionally on the outcome: exactly like every other slot, this one is still handed to [D7](D07-contract-pool-fallback.md)'s cascade, and drops to a lower tier if — and only if — its target tier's candidate pool is empty. D7's own text already anticipated this: *"the guarantee, if D6 adopts one, is 'the slot gets filled at the highest tier the cascade can reach,' not 'the slot gets filled at exactly the assigned tier or is left empty.'"* Every place this document, the GDD or the catalogue says the offer "guarantees" or "always" includes the highest tier, that is shorthand for "targets, subject to D7's fallback" — stated in full once here rather than qualified every time it's said.

**The other two slots' target tier is a weighted, independent draw over the full eligible tier set.** The weight per tier is a `Config` field — `OfferWeight`, one integer per `ContractTier` element of `Config.Contracts` — defaulting to **1 for every tier**, i.e. an even split, until M2's measurement gives a reason to move it. This is Option C's tunability bought at zero cost today: nothing about the default behaviour differs from "uniform," and nothing about §24.2's arithmetic depends on how these two slots land, because the guarantee alone is what the ladder's numbers require. **`OfferWeight` must be a positive integer — zero and negative values are invalid Config, rejected at construction the same way RFC §6.2 already validates `Rounds` rather than trusting the caller.** This isn't a restriction invented for its own sake: it exists specifically so "every eligible tier has zero weight" is a state that cannot occur, rather than a runtime case the weighted draw would otherwise need an undefined fallback for.
...
- **`ContractTier` — the element type of `Config.Contracts`, so the field is addressed as `Config.Contracts[i].OfferWeight` — gains an `OfferWeight int` field**, required positive (construction-time validated, never zero or negative), default `1`, also an implementation-time edit rather than one made in this PR — flagged here so it isn't independently rediscovered.
