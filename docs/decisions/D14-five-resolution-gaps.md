...
- **No new RNG accounting anywhere in this decision — D3's table gains no new row.** Torched is deterministic (hits every post in the sector, no draw). Muscle's loss is a deduction from the melee's own resolution, not a draw. The hand-limit check is arithmetic. Open Doors reuses an existing `market.stock` roll rather than drawing a new one. Bounty's tie-break reuses the existing fairness-key chain: on a round where the RP tie survives Infamy and balance, resolving it *does* spend the existing `confront.tiebreak` draw — the same conditional, fourth-level-only draw RFC §6.4's lazy-draw table already accounts for generically (row: "no confrontation occurs" ⇒ `confront.d6`, `confront.tiebreak` not consumed) and the same one `New Boss` can already trigger. That is a new *call site*, not a new *consumer*: no new purpose string, no new index-count formula, nothing for §16.2's invariant test to learn it doesn't already cover. The issue anticipated cases 4 and 5 might need a new D3 row; tracing both shows they don't.
