...
GDD §6.1 constraint 3 requires four sectors, each holding 4 to 8 nodes — a floor of 16 nodes total. Three node counts the GDD itself names are below that floor: the 15-node two-player map (§6.3), and the 12- and 16-node solo scenario maps (§19.1's *First Run* and *The Trail*). 16 clears the floor only by landing exactly on it. What changes so the generator can be written at all?
...
- **[D3](docs/decisions/D03-rng-consumption-table.md)'s Sinkhole and Scaffolding rows are unblocked.** Both were marked "pending D8, safe only at 20/22/25/28 nodes" — with sector count fixed at four and the minimum at 3, both pools are non-empty (≥3 nodes per sector, always exactly four sectors) at every node count named anywhere. D3's affected passages are updated in this same pull request rather than left to go stale, per this repository's own standing lesson about cross-references that outlive the decision that made them true.
