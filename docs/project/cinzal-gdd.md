# CINZAL
## Game Design Document — v2.32 (scope-locked for prototype)
...
>
> **Changelog v2.24 → v2.25** — R9 measured clear of §22's threshold at 4 players (issue #229)
...
>
> **Changelog v2.29 → v2.30** — §20's R7 remedy dials are rejected, two on measurement and one on structural grounds (issue [#280](https://github.com/garnizeh/cinzal/issues/280), [D42](../decisions/D42-r7-two-player-remedies-rejected.md))
> - **§20's R7 paragraph named three remedies for the 2-player Infamy-9 climb, and none of them can move it.** A D35-rigor paired sweep found extending Tier IV's deadline (6→7 rounds) roughly halves the settled Tier IV abandonment rate the paragraph was watching for, while moving the climb-to-9 headline by less than a tenth of its own measurement noise; raising Tier IV's reward (Cr$ 30/8 → 35/10) moves the headline by exactly nothing — both seeds read bit-identical to the shipped baseline. The third, coupled dial (Legend's step allowance) was not run: it fails the same structural test the first two failed empirically.
> - **The structural reason: all three dials take effect only after Infamy 9 is already reached.** Tier IV requires Infamy ≥ 9 to be offered at all, and Legend's own step row only applies at Infamy 9–10 — neither a contract's terms nor a tier's mobility can influence a decision made before that contract or that tier exists. §20's own framing — *"if nobody voluntarily crosses Infamy 8"* — asks whether a player weighs the post-crossing trade before deciding to keep climbing; no bot tier in `internal/bots` plans that far ahead, so no version of any of these three dials was ever going to be checkable by bot simulation, independent of which values were tried.
> - **R7's 2-player cell is deferred to M5.5**, the same disposition R7's own 3–5 player Tier IV-abandonment note already carries, and the same one D36 gave the lease rate — recorded in issue #241, not in this changelog entry. §22 row 4's band, threshold and read tier are unchanged; this entry does not claim the band is wrong, only that bot simulation cannot validate an incentive aimed at a game state no bot tier anticipates.
> - No other rule or band changes. Companion RFC moves to r36 (pointer only).
>
> **Changelog v2.30 → v2.31** — §22 row 1 stopped being a headless fact when D39's rule shipped, and says so (issue [#283](https://github.com/garnizeh/cinzal/issues/283), [D43](../decisions/D43-row-1-unmeasurable-post-d39.md))
> - **Row 1 is now marked *(M2 → M5.5)*, and the framing count moves from seventeen headless rows to sixteen.** D39's own rule change removed the quantity row 1 counts: a participant whose declared route can no longer be walked keeps the round's remaining steps and spends them blind (§15), so the engine converts what a halt used to cancel instead of losing it. No event the engine emits distinguishes "converted and later spent" from "genuinely never spent" any more, and `internal/telemetry` reports **no value and no verdict** for the row rather than a zero it can never leave.
> - **The threshold is spent, not cleared.** R1's `> 15%` is an *action* — "the confrontation rule gets softened" — and that action was taken, at M2, by D39, on the measurement that tripped it (17.3%–29.1%, Operator, every player count). Reading the same threshold against a post-D39 bot run asks whether to soften a rule that is already the softened one, and the softening is what erased the numerator. The row's band, threshold, denominator and read tier are all unchanged, and a future rule that reintroduces a genuine mid-route cancellation puts a real numerator back in reach.
> - **What is left is a residual, and it is a human question.** A ground-truth probe measured it at **0.18%** of submitted routes with at least one halt (map dead-ends, and incident truncation of an already-converted blind walk) — 80× inside the 15% line, and a map-geometry and incident-deck fact rather than a confrontation one. Whether players *experience* that stall as the lottery R1 named is what M5.5 reads, tracked in issue #241 alongside rows 15 and 18.
> - No other rule, row or band changes. Companion RFC moves to r37 (pointer only).
>
> **Changelog v2.31 → v2.32** — §20's R6, R9 and R11, and §10.4's rate claim, catch up to results M2 already produced (issue [#286](https://github.com/garnizeh/cinzal/issues/286))
> - **R6 gains its M2 result.** Measured at M2 ([#205](https://github.com/garnizeh/cinzal/issues/205)), then re-measured after [D40](../decisions/D40-gas-leak-riot-row-6-and-silent-truncation.md) found Gas Leak and Riot were structurally invisible to row 6 ([#274](https://github.com/garnizeh/cinzal/issues/274)/[#278](https://github.com/garnizeh/cinzal/issues/278)): **25.68% / 32.99% / 39.28% / 44.55%** at 2/3/4/5 players against the 30-60% target band and the `< 20%` / `> 70%` action lines — **NO ACTION at every player count**, none of the three named dials needed. One item still travels to M5.5 via [#241](https://github.com/garnizeh/cinzal/issues/241): conditional on the drawn card being able to hit at all, incidents land at 71.1% at 5 players, above the row's own `> 70%` failing line, and whether row 6 should read that conditional rate instead is a band call D36/D37/D38 all declined to make on bot evidence alone.
> - **R9 gains the measurement its D38 indicator pointer never carried.** At 4 players the remedy was taken: [#229](https://github.com/garnizeh/cinzal/issues/229)/[#234](https://github.com/garnizeh/cinzal/issues/234) raised `MapByPlayers[4]` 25→28 nodes, dropping the rate from 13.35-13.38 to 11.7-11.9 — back inside the 4-12 band. At 5 players the same remedy was measured and declined ([D37](../decisions/D37-five-player-confrontation-load.md)): sweeping 28→52 nodes clears the `> 12` threshold at 46 nodes (11.59-11.62), but by then §22 rows 8 and 19 — the remedy's own headless guardrail — have broken their own bands. **5-player R9 closes as measured-and-deferred to M5.5**, tracked in [#241](https://github.com/garnizeh/cinzal/issues/241).
> - **R11 gains its verification result.** Verified at M2 ([#203](https://github.com/garnizeh/cinzal/issues/203)): **NO ACTION at every player count** — every interval sits comfortably inside the < 30% target band (roughly 21% at 2-3 players, down to 20.5% at 5), nowhere near the 45% failing line even at its tightest cell. Both candidate remedies stay correctly unwritten, per R11's own "verify it happens first." Standing caveat carried to M5.5 via [#241](https://github.com/garnizeh/cinzal/issues/241): Operator has no concept of being out of contention, so this is weak evidence for human play.
> - **§10.4's "note on the rate" gains D36's finding.** The claim that the lease rate is *"the single most sensitive dial in the game"* was tested directly: [#204](https://github.com/garnizeh/cinzal/issues/204) swept `LeaseCostPerBlock` (0-20) × `LeaseBlockRounds` (1-15) across 106 configurations at 2/3/4/5 players, every point at least an order of magnitude below §22 row 7's `< 1` failing floor. [D36](../decisions/D36-lease-rate-chokepoint-gate.md) then showed the rate isn't the binding constraint: even fully loosening Operator's own chokepoint gate — a bots-package tuning pair, not the lease rate — only moves the measured range from 0.00175 to 0.28387 leases/player, still 7-14× short of the 2-4 band, at a real, measured RP cost. `game.Config` and `OperatorOptions` ship unchanged; the row closes as measured, out of band across the tested range, deferred to M5.5 via [#241](https://github.com/garnizeh/cinzal/issues/241).
> - **§10.3's cap table still read 25 nodes for 4 players; corrected to 28.** v2.25 raised §6.1's 4-player row to 28 nodes for R9 but never propagated the change to §10.3's own copy of the node count — a stale-table bug this entry's R9 paragraph would otherwise have contradicted directly. Nodes only; the post cap (4) and max posts (16) are unchanged, so the coverage percentage moves from 64% to 57%.
> - **No band, threshold or rule changes.** This records what M2 already measured, plus the one stale-table correction above; none of it is re-litigated here. Companion RFC moves to r39 (pointer only — every citation above is GDD-owned data or an already-decided disposition, not a new RFC-stated structure).
...
**This was expected to be the single most sensitive dial in the game** — the one number paper testing would move first, kept as configuration rather than a constant for that reason. **That expectation was tested at M2, and it did not survive ([#204](https://github.com/garnizeh/cinzal/issues/204), [D36](../decisions/D36-lease-rate-chokepoint-gate.md)).** A sweep of `LeaseCostPerBlock` (0-20) × `LeaseBlockRounds` (1-15) across 106 configurations at 2/3/4/5 players put every measured point at least an order of magnitude below §22 row 7's `< 1` failing floor — the rate itself was never close to the target band regardless of where it was set. D36 then found the rate isn't the binding constraint: even loosening Operator's own chokepoint gate (`ChokepointTrafficRate`/`ChokepointMinObserved`, a `internal/bots`-internal tuning pair, not the lease rate) all the way to its most permissive setting only moved the measured range from 0.00175 to 0.28387 leases/player — still 7-14× short of the 2-4 band — at a real, measured RP cost to the bot using it. `game.Config` and `OperatorOptions` ship unchanged; the row closes as measured, out of band across the tested range including the gate's own removal, deferred to M5.5 via [#241](https://github.com/garnizeh/cinzal/issues/241) rather than answered by a `Config` edit.
...
**Why this shape.** The cost of losing is the round's *payoff* and, usually, a cargo — painful, recoverable, and over. Precisely: the declared route, the action it was built to reach, the stake, a point of Infamy, one or two nodes of ground, and the cargo unless the loser went Evasive and could pay the shakedown in full. What it is **not**, since [D39](../decisions/D39-r1-confrontation-softening.md), is a round spent standing still: the remaining steps are still walked, blind, from wherever the fallback leaves you. Losing costs you what you were trying to do, not your ability to do anything at all — which is the distinction R1 (§20) turned out to be about. It is not the v0.9 original, where a loser paid stake plus fee plus Infamy plus a base. The **underestimated** modifier is the comeback valve: the player everyone has stopped watching gets a real shot at the one who thinks they own the city.
...
**And the threshold is now spent rather than clearable — §22 row 1 cannot be re-read against it ([#283](https://github.com/garnizeh/cinzal/issues/283), [D43](../decisions/D43-row-1-unmeasurable-post-d39.md)).** This paragraph's `> 15%` is an *action*, and the action was taken: the rule was softened, above, at M2. Softening it is also what removed row 1's numerator — under §15 as it now reads, the engine converts a halted participant's remaining steps into a blind walk instead of cancelling them, and no event it emits separates "converted and later spent" from "genuinely never spent." Row 1's numerator is therefore a constant 0 at every player count and every tier — which §22's own reading rule calls a degenerate sample, not a finding — and that is why post-D43 `internal/telemetry` reports **no value and no verdict** for the row at all, rather than the zero it can never leave. The `0.000000` that M2's CSVs carry is the pre-D43 output, and it was never a pass. A ground-truth probe put the residual — map dead-ends, and incidents truncating an already-converted walk — at **0.18%** of submitted routes with at least one halt, 80× inside this paragraph's line and a map-geometry and incident-deck fact rather than a confrontation one. Whether that stall is *felt* as the lottery this paragraph describes is a human question, deferred to M5.5 and tracked in [#241](https://github.com/garnizeh/cinzal/issues/241). The 15% line, the denominator and the read tier all stay as written, so a future rule that reintroduces a genuine mid-route cancellation makes the row measurable again without reopening this decision.
...
**R6 — RESOLVED, no action needed.** Thirteen incidents in a 15-round match is a lot of chaos to layer on top of a global event every round. Watch for players reporting that routes feel unplannable. Dials in order: start at round 5; odd rounds only; cut the deck to the six least punishing.

Measured at M2 ([#205](https://github.com/garnizeh/cinzal/issues/205)), then re-measured after [D40](../decisions/D40-gas-leak-riot-row-6-and-silent-truncation.md) found Gas Leak and Riot were structurally invisible to §22 row 6 — both cards resolve outside `incident()`'s own switch, so neither emitted `EventIncidentHit` until [#274](https://github.com/garnizeh/cinzal/issues/274)/[#278](https://github.com/garnizeh/cinzal/issues/278) fixed both call sites: **25.68% / 32.99% / 39.28% / 44.55%** of incidents actually hit a player at 2/3/4/5 players, against the 30-60% target and the `< 20%` / `> 70%` action lines — **NO ACTION at every player count**, comfortably inside the band at the tightest cell (2 players, clearing the 20% floor by 5.53 points). None of the three named dials was needed. One thing still travels to M5.5, tracked in [#241](https://github.com/garnizeh/cinzal/issues/241): conditional on the drawn card being one that can hit at all, the rate reaches 71.1% at 5 players, over the row's own `> 70%` line — whether row 6 should read that conditional rate instead of the unconditional one is a band call D36, D37 and D38 all declined to make on bot evidence alone.
...
Watch for Tier IV contracts being accepted and then abandoned. If nobody voluntarily crosses Infamy 8, the dials were meant to be: extend the Tier IV deadline to 7 rounds; raise Tier IV to Cr$ 35 / 10 RP; or raise Legend to 3 steps.

**All three are rejected — the first two on measurement, the third on structural grounds without needing a run — at M2 ([#280](https://github.com/garnizeh/cinzal/issues/280), [D42](../decisions/D42-r7-two-player-remedies-rejected.md)).** The 2-player Infamy-9 climb tripped the `< 10%` action threshold post-[D39](../decisions/D39-r1-confrontation-softening.md) (8.93% pooled, confirmed on two independent seed pairs). Extending Tier IV's deadline roughly halves the settled abandonment rate this paragraph was watching for, but moves the climb-to-9 rate by less than a tenth of its own measurement noise; raising Tier IV's reward moves it by exactly nothing. Both measured null results share one cause, which is also why the third, coupled dial (Legend's step allowance) was never run: Tier IV requires Infamy ≥ 9 to be offered at all, and Legend's own step row only applies at Infamy 9–10 — none of the three can influence a decision made *before* the contract or the tier they govern exists to be reasoned about. This paragraph's own "if nobody voluntarily crosses Infamy 8" is a rational-actor question about weighing the post-crossing trade in advance; no bot tier in `internal/bots` plans that far ahead, so no simulated sweep of any of these three dials was ever checkable, independent of which values were tried. The 2-player cell is deferred to M5.5, tracked in [#241](https://github.com/garnizeh/cinzal/issues/241).
...
**The remedy was measured, and it splits by player count.** At 4 players it was taken: [#229](https://github.com/garnizeh/cinzal/issues/229)/[#234](https://github.com/garnizeh/cinzal/issues/234) raised `MapByPlayers[4]` from 25 to 28 nodes, dropping the measured rate from 13.35-13.38 to 11.7-11.9 confrontations per match — back inside the 4-12 band. At 5 players it was measured and declined ([D37](../decisions/D37-five-player-confrontation-load.md), issue [#232](https://github.com/garnizeh/cinzal/issues/232)): a 28→52 node sweep clears the `> 12` threshold at 46 nodes (11.59-11.62), but by 46 nodes the remedy's own guardrail has broken — §22 row 8 (share of map under sight) falls to 0.281, out of its 30-55% band, and row 19 (Heat Map entries at low confidence) rises to 0.445, out of its `< 40%` target. Reaching 46 nodes would also mean reopening [D8](../decisions/D08-sector-size-constraint.md)'s per-sector cap and [D10](../decisions/D10-map-layout.md)'s layout lattice. **5-player R9 closes as measured-and-deferred to M5.5**, tracked in [#241](https://github.com/garnizeh/cinzal/issues/241).
...
**R11 — RESOLVED, no action needed.** In the last two or three rounds a trailing player can no longer complete a contract, and switching to farming confrontations on a chokepoint becomes rational — Infamy and stakes with no opportunity cost left to pay. Loitering makes it visible, which is most of the fix, but visibility doesn't help the player being kingmade against. Candidate remedies if it shows up: confrontation Infamy gains halved in the final two rounds; or stakes returned rather than transferred once contracts can no longer be delivered. Do not pre-emptively patch this — verify it happens first.

Verified at M2 ([#203](https://github.com/garnizeh/cinzal/issues/203)): **NO ACTION at every player count.** Confrontations in the final 3 rounds run roughly 21% of all confrontations at 2-3 players, down to 20.5% at 5 — every interval comfortably inside the `< 30%` target band, nowhere near the `> 45%` failing line, the tightest cell (5 players) clearing the band by almost 10 points. Per this paragraph's own instruction, both candidate remedies stay correctly unwritten. **Standing caveat, carried to M5.5** via [#241](https://github.com/garnizeh/cinzal/issues/241): Operator has no concept of being out of contention, so it will never rationally farm a chokepoint out of spite the way a losing human might — this result says the mechanic doesn't manufacture endgame camping on its own against a bot that's still nominally trying, and is weak evidence for human play.
...
R1 and R9 are answered by measurement, not opinion — R2 already was, and turned out to be backwards (§20). Every match — paper or digital — records the following, with one caveat [D33](../decisions/D33-telemetry-event-stream-coverage.md)'s row-by-row audit made explicit: sixteen of the twenty rows are headless facts, computable from a running match's event stream, final state, or order log alone; four are not, and are marked below with the milestone that produces them instead. Three of those four never were headless — rows 15, 16 and 18 are UI or human questions ([D33](../decisions/D33-telemetry-event-stream-coverage.md)). The fourth, **row 1, stopped being one**: [D39](../decisions/D39-r1-confrontation-softening.md)'s rule change converts what a halt used to cancel, leaving no event the engine can emit to count against it, so the digital build reports no value and no verdict for that row and its read moves to M5.5 ([D43](../decisions/D43-row-1-unmeasurable-post-d39.md)). The digital build computes each row from its own declared source, not one uniform mechanism: event-backed rows are emitted as structured events from day one, because retrofitting instrumentation always costs more than building it in; final-state and order-log rows are computed once, at scoring, through `telemetry.Match` (RFC §17). Row 13 ships without a precise answer in M2's first pass regardless of source — marked open below, not silently approximate.
...
Rows marked *(M5)* or *(M5.5)* below have no headless statistic to reduce and sit outside this rule entirely. Row 1 is the one row the zero-width bullet above actually caught. Its M2 CSVs read a constant 0 at every cell — the pre-D43 output — because the rule change R1 itself forced (D39) removed what the row counted; that is a degenerate sample, not a pass. **Post-D43 the row emits no value and no verdict at all**, so there is no longer a zero for a reader to mistake for one — D43, and the marking on the row.
...
|---|---|---|---|
| 1 | Routes cancelled mid-route *(M2 → **M5.5**; one per **route**, never one per halt event — [D39](../decisions/D39-r1-confrontation-softening.md); no headless numerator survives D39's own rule change, so M2 reports no value and no verdict — [D43](../decisions/D43-row-1-unmeasurable-post-d39.md))* | < 15% of submitted routes | **> 15%** → confrontation too punishing (R1) — an *action* already taken by D39, not a line still waiting to be crossed |
