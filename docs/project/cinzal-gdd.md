...
> - **R6 gains its M2 result.** Measured at M2 ([#205](https://github.com/garnizeh/cinzal/issues/205)), then re-measured after [D40](../decisions/D40-gas-leak-riot-row-6-and-silent-truncation.md) found Gas Leak and Riot were structurally invisible to row 6 ([#274](https://github.com/garnizeh/cinzal/issues/274)/[#278](https://github.com/garnizeh/cinzal/issues/278)): **25.68% / 32.99% / 39.28% / 44.55%** at 2/3/4/5 players against the 20-70% band — **NO ACTION at every player count**, none of the three named dials needed. One item still travels to M5.5 via [#241](https://github.com/garnizeh/cinzal/issues/241): conditional on the drawn card being able to hit at all, incidents land at 71.1% at 5 players, above the row's own `> 70%` failing line, and whether row 6 should read that conditional rate instead is a band call D36/D37/D38 all declined to make on bot evidence alone.
...
> - **No band, threshold, rule or number changes.** This records what M2 already measured; none of it is re-litigated here. Companion RFC moves to r39 (pointer only — every citation above is GDD-owned data or an already-decided disposition, not a new RFC-stated structure).
