...
> - **§9.4's Bolt Hole now states what its "2 steps away" is measured from: the player's own position at the start of the round** — the one coordinate both player and server agree on before any order resolves, and the only one available at the moment the node is declared, before that round's routes and confrontations exist. The declared node must itself be Known, not merely Rumoured — a Rumoured node carries no edges to route into (§7.1), so it cannot be the endpoint of a 2-step path in the first place.
