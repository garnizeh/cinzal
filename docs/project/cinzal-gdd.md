...
Rows marked *(M5)* or *(M5.5)* below have no headless statistic to reduce and sit outside this rule entirely. Row 1 is the one row the zero-width bullet above actually caught: it reads a genuine constant 0 at every cell because the rule change R1 itself forced (D39) removed what it counted, which is a degenerate sample and not a pass — D43, and the marking on the row.
