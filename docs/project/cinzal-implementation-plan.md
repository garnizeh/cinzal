...
- The **16 of 20** per-match §22 rows that are headless facts, plus the per-round and per-action sets. Rows 15 and 18 belong to M5.5 and row 16 to M5; row 13 ships without a precise answer and says so. **Row 1 is computed and reports no value**: [D39](../decisions/D39-r1-confrontation-softening.md)'s own rule change converted away the quantity it counts, so `telemetry` returns an empty `Rate` for it — no mean, no interval, no verdict — and R1's read moves to M5.5 ([D43](../decisions/D43-row-1-unmeasurable-post-d39.md)). That is the count's seventeenth row, and the reason it is no longer one of the sixteen.
