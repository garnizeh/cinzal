...
> - GDD §15 now lets a confrontation loser — and every tie participant — spend the round's remaining steps as §9.1 blind steps from their fallback node instead of freezing. **§6.4's consumption table is unchanged**: no new purpose string, and `pushon.edge`/`scavenge.d6` already read "1 per blind step" and "1 per newly explored node". What changes is §6.4's second worked case, whose "a loser who moved consumes 0" line was true only while a loser stopped moving. The draws stay lazy, so §16.2's index-count invariant is asserted the same way against a different predicted count.
...
- A **stationary Evasive loser** consumes **2** `pushback.hop` indices. A stationary Neutral loser consumes 1. A loser who moved consumes **0** *for the fallback itself*, because it walks a known route rather than drawing — but every loser and every tie participant then spends whatever the round's step allowance had left as blind steps from where they end up ([D39](../decisions/D39-r1-confrontation-softening.md), GDD §15), so the `pushon.edge`/`scavenge.d6` pair above fires once per those steps as well. No new purpose string: this is the existing blind-step consumer reached from a second place, and it stays lazy — a loser with no steps left draws nothing, and a boxed-in blind step draws nothing, exactly as §9.1's own walk does.
