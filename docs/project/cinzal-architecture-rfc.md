...
>
> **Changelog r20 → r21** — `globalEvent()` implemented (issue #72), extending r19's "peek early, announce late" pattern past Rain
> - **Curfew, Gates Closed and Permit Auction are deterministic once the card's identity is known — no draw, so re-reading `eventCardThisRound`'s peek at `validate` (Curfew), `resolveDeliveries` (Gates Closed) and `resolveAddons`/`resolveActions` (Permit Auction, which discounts a lease block's cost at both a fresh stake and a renewal — GDD §14.2 says "lease blocks," not "renewals only") is exactly Rain's own r19 shape, reused three more times, no new §6.4 row.**
> - **Dragnet, Shipping Boom and Festival are the first cards to need a genuine pipeline change.** Each draws a real target (`event.dragnet`, `event.shippingboom`, `event.festival`) that an earlier pipeline step consumes this same round — a sealed Border blocks `Deliver` at `validate`, an overloaded Warehouse's bonus pays out inside `resolveActions`'s Pickup branch, and Festival's "leaves no trace" needs its node before `writeTrail` runs — all strictly before `globalEvent()`'s own Phase 6 call site. The draw itself moves to the same round-start peek `buildGlobalEventContext` already performs for the three deterministic cards above; `globalEvent()` reuses the already-drawn value at Phase 6 to announce it, never redrawing. §6.4's Phase column for these three rows now means "revealed," not "drawn" — the draw's position in the round's `seq` sequence is unchanged, since nothing computed it any differently before this issue actually built the call site.
...
>
> **Changelog r22 → r23** — two sector incidents had no fog-writer row (D26)
> - **§9.1's table gains rows 15 and 16.** `EventInformantRing` (Informant Ring, GDD §14.3) and `EventSpilledLoadCrate` (Spilled Load, GDD §14.3) landed with #73's sector incidents, after rows 13-14 were added, and were never given a row. "Fourteen authorised writers" is now sixteen throughout §9.1, §16.1's Fog test row count, and the Anchor-parity test's per-row citation list.
...
> - **Two legality gaps `Legal` never checked**: a negative `Stance.Stake` (`game.Order.Stake`'s own doc comment: "Cr$ 0-6") and a `PushingOn.Steps` count outside GDD §9.1's declared 0-2 range. Neither is its own row in GDD §15.0's table — issue #79 names both explicitly as malformed shapes the table doesn't enumerate "because they are not rules," i.e. already fully determined by field documentation elsewhere, not new legality decisions. The stake gap is more than untested: `resolveLoser`'s `staked := min(o.Stance.Stake, p.Balance)` goes negative too, and deducting a negative amount *increases* the loser's balance on a loss. New `Reason` values `ReasonNegativeStake` and `ReasonBlindStepCountExceeded` (legal.go).
> - **Every involuntary relocation except pushback skipped Fog entirely.** GDD §7.2: "a visited node becomes Known permanently," unconditional; GDD §15's Displacement rule: sight follows "wherever you actually end the round... however you got there." Ordinary movement (`advance`, movement.go) already goes through `scavenge`, which marks the arrival node Known. `pushback` (confront.go) and three sector-incident relocations (`resolveFlood`, `resolveSnatchJob`, `resolveTurfWar`, incidents.go) set `Position` directly and never touched `Fog` — a player could end a round standing on a node their own client would still render as Hidden. Caught by the property sweep within the first handful of rounds at every player count, once it actually drove random orders through confrontations and incidents rather than the empty-orders loops every prior "many seeds" test in the suite used. Fixed with one shared helper, `markNodeKnown` (movement.go), used by `scavenge` and all four relocation sites — deliberately without a Scavenging roll, since GDD §9.1 ties that roll to a deliberate, step-spending exploration choice, not a displacement nobody chose.
> - No GDD text changes anywhere in this entry — all four bugs contradicted already-correct spec language; this is the code catching up to it, not the reverse. Companion doc stays at v2.22.
> - §16's testing table gains its last two previously-unimplemented layers: `rules` property (§16.2's seven state invariants, ≥1000 randomised matches, 2-5 players — `internal/rules/property_test.go`) and Adversarial (one test per GDD §15.0 row plus the malformed shapes above — `adversarial_test.go`), plus §16.1's cross-round-state row for the fields #74 (upkeep, the write side) didn't already cover — one test per §6.6 field, framed on firing round rather than counter movement (`crossround_test.go`).
> - One golden fixture hash moves (`determinism_test.go`'s 5-player fixture): its scripted order log exercises an Evasive confrontation loss, so the shakedown-transfer fix changes its final state. Deliberate, not a regression — see this entry.
>
...
> - **§11's `POST /matches` entry states the raw-token handoff**: since `invite_links` stores only `token_hash`, the creation handler is the only place the raw token ever exists outside a client's URL bar, and it must return the invite URL in that same response — generate-once, show-once, the same shape as any other server-minted bearer credential.
...
// categoryOf/hazardOrBoon and quota are each pool's own category function
// and per-category count — see internal/rules/cards.go for the real ones.
eventDeck    = ShuffleConstrained(rng, PurposeDeckEventSelect,    PurposeDeckEventOrder,    allEvents,    categoryOf, quota)    // 12 of 24: 3 per category
incidentDeck = ShuffleConstrained(rng, PurposeDeckIncidentSelect, PurposeDeckIncidentOrder, allIncidents, hazardOrBoon, quota) // 13 of 16: 9 hazards, 4 boons
```
...
-- the bearer credential for admission to one match's lobby (D17)
invite_links(
...
**Opponent position information has exactly sixteen authorised writers**, and the projection must implement all of them. The Board's anchored Attribution (GDD §7.5) is built entirely on this table; omit a row and the deduction layer has nothing to intersect.

**Not every row comes from GDD §7.3's trail table, and the parity check must not assume it does.** Rows 1–5, 7 and 8 map one-to-one onto seven of §7.3's eight event types (the eighth, Fresh tracks, is never named and so has no row here by design — it is position information without a name attached, out of scope for this specific table). Rows 6, 9, 10 and 11 are sourced from other GDD sections — loose crate holding (§8.4), the Infamy ladder's tier reveals (§11), and the Informants event (§14.2); row 12 is sourced from [D12](../decisions/D12-decoy-fog-writer.md), itself grounded in GDD §9.4 and §12; rows 13 and 14 (issue #72) are sourced from GDD §14.2 directly — Dead Runner's and Fence's Windfall's own "announced publicly" card text; rows 15 and 16 ([D26](../decisions/D26-sector-incident-fog-writer-rows.md)) are sourced from GDD §14.3 directly — the Informant Ring and Spilled Load sector incidents' own card text. None of the nine is a row of §7.3's table. The check whenever either document moves is therefore two-part: every §7.3 row with a name attached must map to exactly one row here with matching sight-gated/global and named/node-only semantics, **and** every row here without a §7.3 counterpart must cite its source document and section instead — a GDD section directly for rows 6, 9–11 and 13–16, or D12 plus its underlying GDD §§9.4 and 12 for row 12. A literal row-count or row-for-row equality check between the two tables was never correct — it would have failed at eleven rows against §7.3's eight, before D12 added a twelfth.
...
| 13 | **Dead Runner** event, crate placed | Global, once | **No** | Yes — the drawn node |
| 14 | **Fence's Windfall** event, standing offer opens | Global, once | **No** | Yes — the drawn Black Market |
| 15 | **Informant Ring** sector incident ([D26](../decisions/D26-sector-incident-fog-writer-rows.md)) | Global, once, per eligible seat | Yes | Yes |
| 16 | **Spilled Load** sector incident, crate placed ([D26](../decisions/D26-sector-incident-fog-writer-rows.md)) | Global, once | **No** | Yes — the drawn node |
...
A finished match's replay bundle is `{seed, config, players, orderLog}` — a few kilobytes. This names the bundle's four logical inputs, not literal JSON keys; the actual wire encoding is whatever `cmd/replay/bundle.go`'s own `Bundle` struct explicitly tags each field as (`seed`, `config`, `players`, and `order_log` for the fourth — Go's `encoding/json` has no automatic snake_case convention of its own, so `order_log` is a deliberate tag choice, not an inferred one), and `readBundle`'s `DisallowUnknownFields()` check validates strictly against those tagged names. The browser folds it with the same WASM binary and scrubs through rounds locally. No replay endpoints, no server-side rendering of past states, no load from someone sharing a match on social media.
...
POST /matches                   create (config, player count, timer/deadline, bot seats); mints invite_links'
                                 first row and returns the raw-token invite URL — the only response that ever
                                 carries the raw token (D17)
...
-- This statement runs inside TX_limits (D50) — a short transaction holding
-- only the two bucket consumes, never the rest of the handler — but that
-- transaction can still be held open by lock contention on this row from a
-- concurrent request against the same key; a backdated updated_at from
-- now() would let a later check over-credit the refill.
...
**Durability requirement (D50):** this consume must commit before the handler does anything else, and must never be the return value of a transaction closure that something *unrelated* in the same transaction could roll back. Reusing `Tick()`'s `m.tx(ctx, func(q *store.Queries) error { … })` idiom to couple it to the request-specific write that follows — return an error to signal denial, let the non-nil return trigger `ROLLBACK` — would un-do the consume itself the moment that later write fails for reasons that have nothing to do with the rate limit, which a spent token must survive instead. But the idiom is still exactly right for coupling the consume to its **own sibling**: both `/auth/request` and `/auth/verify` run `TX_limits`, one `m.tx` holding only the `auth_email` and `auth_ip` consumes, where a non-nil return on either still rolls back both — that coupling is the cross-axis guarantee itself, not the bug. Nothing has been granted yet at that point, so an abort there, from any cause including a client disconnect, is a clean denial rather than a refund. `TX_limits` committing is what gates everything else: the request-specific write (`auth_codes` insert + outbox enqueue for `/auth/request`; the code check and, on a match, session creation for `/auth/verify`) runs afterward in its own ordinary, cancellable `m.tx`, since nothing durable is lost if it rolls back — a retry costs another already-committed token, not a free one. `/auth/verify`'s wrong-code branch is the one write that needs a different shape again: `auth_codes.attempts` increments as a standalone statement on a context built with `context.WithoutCancel`, because by the time it runs the code has already been compared in memory, so a disconnect there would let an attacker discard the record of an attempt whose outcome they already have — a gap `TX_limits`'s all-or-nothing failure mode doesn't share. See D50 for the full per-outcome enumeration and why the two counters need different treatment despite both being durability-critical.
...
| Anchor parity | For each of GDD §7.3's eight trail-entry types, assert it maps to its correct row of §9.1's sixteen-writer table (or correctly to no row, for Fresh tracks) with matching named/sight-gated semantics; separately assert rows 6, 9–11 each cite their GDD section, row 12 cites D12 plus GDD §§9.4 and 12, rows 13–14 cite GDD §14.2 directly, and rows 15–16 cite GDD §14.3 directly ([D26](../decisions/D26-sector-incident-fog-writer-rows.md)). A table-driven test over both lists, so a change to either fails the build. |
...
```text
docker build → one binary + one image (ENV TZ=UTC)
env: DATABASE_URL, MAIL_PROVIDER_KEY, BASE_URL, SESSION_KEY, TRUSTED_PROXY_HOPS
start: migrate (advisory lock) → serve
```

`ENV TZ=UTC` is a harmless backstop, not a dependency (D55) — no code path
branches on the process's `TZ`; §7.5's session pin and §17's `slog` fix are
what correctness actually rests on, and both hold identically whether or
not this is set.
