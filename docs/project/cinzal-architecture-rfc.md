...
**This is fog-sensitive state living inside `MatchState`.** Seat A's archive must never reach seat B, so `Project` ships only the requesting seat's `Sight` and `Obscured`, and the fog suite tests both as it tests any other hidden fact.
