...
>
> **Changelog r11 → r12** — the Go version is pinned, not a floor
> - **§4 now names Go 1.26.5** rather than "1.23+". A floor invites the toolchain to drift between contributors and between a developer's machine and CI, and this is the one project where that class of drift is expensive: §6.3 stakes the whole design on `seed + order log` reproducing a match **exactly, forever**, and §15.4 alerts loudly when a refold disagrees with the incrementally computed state. A determinism mismatch is the one alert §17 says is worth waking someone for, and "which Go built it" should never be among the candidate explanations.
> - The four hazards in §6.3 are all in our own code and none of them are version-dependent, so this pins a variable rather than fixing a known bug. That is the point: the cost of pinning is a `go.mod` line, and the cost of not pinning is paid only once, in the worst possible week.
>
...
`rules` imports nothing but the standard library, and from the standard library it does not import `time`, `math/rand`, `os`, or anything touching the network. This is enforced by a CI check, not by convention.
...
- **`rules` cannot reach a provider even by accident**, because it imports nothing outside the standard library (§6.1) and the CI check enforces it. The compiler makes this class of bug unavailable rather than merely discouraged.
