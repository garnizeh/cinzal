...
**Not every row comes from GDD §7.3's trail table, and the parity check must not assume it does.** Rows 1–5, 7 and 8 map one-to-one onto seven of §7.3's eight event types (the eighth, Fresh tracks, is never named and so has no row here by design — it is position information without a name attached, out of scope for this specific table). Rows 6, 9, 10, 11 and 12 are sourced from other GDD sections entirely: loose crate holding (§8.4), the Infamy ladder's tier reveals (§11), the Informants event (§14.2), and Decoy ([D12](../decisions/D12-decoy-fog-writer.md), itself grounded in §9.4 and §12) — none of which is a row of §7.3's table. The check whenever either document moves is therefore two-part: every §7.3 row with a name attached must map to exactly one row here with matching sight-gated/global and named/node-only semantics, **and** every row here without a §7.3 counterpart must cite the specific GDD section it comes from instead. A literal row-count or row-for-row equality check between the two tables was never correct — it would have failed at eleven rows against §7.3's eight, before this decision added a twelfth.
...
| Fog | Negative assertions (§16.3 below), plus one positive and one negative test per row of the twelve-writer anchor table (§9.1), including the Decoy-vs-real-pickup indistinguishability test ([D12](../decisions/D12-decoy-fog-writer.md)). |
