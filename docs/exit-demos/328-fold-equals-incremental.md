...
1. **Build the fixture.** `buildIncrementalFixture` drives a full 15-round match: `rules.NewMatch(seed, cfg, players)`, then per round, every seat's order via `rules.ProjectView` + `bots.For(bots.Operator).Decide` + `rules.NewBotRNG(seed, seat, round)`, then `rules.Resolve`. This is the incremental path — the same shape `cmd/simulate/driver.go`'s `RunMatch` uses. The resulting order log and final `MatchState` are both kept.
2. **Golden comparison.** The incremental final state is marshalled to JSON and compared byte-for-byte against the committed `testdata/fold-equivalence-{2,4}p.json` — this is what would catch a future rule change moving the result even though the same seed always regenerates an identical order log.
3. **Persist through the real write path.** `Store.CreateMatch` + one `Store.AppendOrder` call per `(round, seat)`, against a real Postgres transaction (`storetest.Container`, D46 tier 1) — the same write path a live tick would use, `store.SourceBot` since the log is bot-authored.
4. **Reload through a real SELECT.** `Store.LoadMatch` + `orderlog.Load` — never the in-memory log step 1 built — then `internal/match/fold.Fold(seed, cfg, players, reloadedLog)`.
...
--- PASS: TestIntegrationFoldEqualsIncrementalMatchesGoldenFixture (4.00s)
    --- PASS: TestIntegrationFoldEqualsIncrementalMatchesGoldenFixture/2p (3.75s)
    --- PASS: TestIntegrationFoldEqualsIncrementalMatchesGoldenFixture/4p (0.25s)
=== RUN   TestIntegrationFoldDivergesWhenOrderCorrupted
--- PASS: TestIntegrationFoldDivergesWhenOrderCorrupted (0.09s)
PASS
ok  	github.com/garnizeh/cinzal/cmd/replay	4.156s
```
...
The golden fixtures themselves record a genuine 15-round completion at both player counts — `Round: 15` and the expected seat count — confirmed when they were generated (`-update` run, no database needed for that half, since it only exercises the incremental path).
