...
## Update (#283/D43): the 0.00% column is not a measurement, and this document read it as one

**This document's R1 verdict is withdrawn and replaced.** Everything below about D39's *rule* is unchanged and still stands — rows 2, 9, 10/11, 17 and 20 all reproduce D39's prediction closely, and that is what confirms the rule works. What is withdrawn is the reading of row 1's own column. [#283](https://github.com/garnizeh/cinzal/issues/283) found that this document and the harness that produced its CSVs disagree about the same four bytes: `cmd/simulate` prints `RoutesCancelledMidRoute_mean = 0.000000` with an **empty** `half_width`, which is [#249](https://github.com/garnizeh/cinzal/issues/249)'s own gate saying *"this is not a measurement to read a threshold verdict off of"* (`run.go`), and GDD §22's reading rule says the same in the document — *"a zero-width interval is a degenerate sample, not a finding."* The paragraphs below instead read it as *"a genuine measurement, not an unmeasured gap"* clearing the `< 15%` line with margin.

[D43](../decisions/D43-row-1-unmeasurable-post-d39.md) resolves it in the harness's favour. Since #270, `telemetry.routesCancelledMidRoute` returns a literal `Rate{Value: 0}` — the row is structurally incapable of being non-zero for any config, any seed, any player count — because D39's own rule change converted away the quantity it counts. **A metric that cannot take any other value has not cleared a threshold; it has become unfalsifiable.** R1's `> 15%` is an *action* ("the confrontation rule gets softened"), and that action was already taken by D39 on the measurement that tripped it; re-reading the line afterwards asks whether to soften a rule that is already the softened one.

**So R1's correct disposition, as of D43, is: no measurement, no verdict, read deferred to M5.5** — recorded in [#241](https://github.com/garnizeh/cinzal/issues/241) alongside the lease rate, R9 at 5 players and R7 at 2 players — not "closed, no action." The residual this section's own probe found (0.18% of submitted routes with at least one halt: map dead-ends, and incidents truncating an already-converted blind walk) is real, 80× inside the 15% line, and is what M5.5 reads against human experience rather than against a bot CSV. `internal/telemetry` stops printing the 0 at all ([#289](https://github.com/garnizeh/cinzal/issues/289)).

Two sentences below are marked **[withdrawn — D43]** where they state the superseded reading. They are left in place rather than deleted, per this repository's own "a decision that turns out wrong is superseded, not edited."
...
**R1's underlying rule is confirmed working, again.** `#267` is resolved; `#269`, the boundary defect it surfaced, is resolved with it, and every corroborating row above reproduces D39's prediction closely. **[withdrawn — D43]** ~~and now so is its telemetry. Row 1 reads exactly 0.00% at every cell, `N=10000` throughout — a genuine measurement, not an unmeasured gap — clearing the `< 15%` threshold with more margin than D39's own E+ prediction... GDD §20's R1 entry and §22 row 1 can both be read against this figure going forward~~ — row 1's own column is a constant no input can move, which is a degenerate sample and not a verdict ([D43](../decisions/D43-row-1-unmeasurable-post-d39.md)); §20's R1 entry and §22 row 1 are read against D43 instead, and R1 is deferred to M5.5 in [#241](https://github.com/garnizeh/cinzal/issues/241). Every number in the sections below still predates the numerator fix and the boundary fix alike.
