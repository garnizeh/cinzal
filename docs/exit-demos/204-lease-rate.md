...
> **Update, 2026-08-20 — that mechanism is fixed ([#236](https://github.com/garnizeh/cinzal/issues/236)); the `LeaseBlockRounds=1` column above no longer reproduces.** `fundNewStake` now sizes a fresh stake's up-front blocks to outlive the staking round's own Upkeep at any `LeaseBlockRounds`, instead of assuming the one block that only happens to suffice at the shipped default of 3. Re-measured at the same root seed, 10,000 matches, Operator, against the same rounds sweep: `LeaseBlockRounds=1` reads 0.0015 [.0010,.0020] at 2p, 0.0013 [.0009,.0017] at 3p, 0.0011 [.0008,.0015] at 4p, 0.0006 [.0004,.0008] at 5p — every interval now excluding zero. **Every `LeaseBlockRounds >= 2` cell is bit-identical to the CSVs in [`204/`](204/)** at 2p, 3p and 5p, confirming the fix touches nothing but the one-round case; the 4p row moved for an unrelated reason ([#234](https://github.com/garnizeh/cinzal/issues/234) raised the 4-player node count after this demo was taken), which is why the tables above are read at their stated git SHA, not against today's `main`. **None of this changes any verdict here or in [D36](../decisions/D36-lease-rate-chokepoint-gate.md)** — the repaired column lands at 0.0006–0.0015, still two to three orders of magnitude below GDD §22's `< 1` failing floor, and §Root cause's actual bottleneck (`OperatorOptions.ChokepointTrafficRate`/`ChokepointMinObserved`) is untouched.
