...
2. **Crash-mid-boot recovery** (`TestIntegrationMigrateBootRaceSecondProcessRecoversAfterFirstIsKilled`). §7.5 never names this case, but it is the one an operator meets at 3am: a process dies while holding the lock. `TestIntegrationMigrateReleasesLockOnMidMigrationFailure` (`migrate_integration_test.go`) already proves the *graceful*-error release path (a migration fails, `defer releaseMigrationLock` runs). This test proves the *ungraceful* path instead: process A is started, polled until `pg_locks` shows it holding the advisory lock, then sent `SIGKILL` — no `defer` runs, no message is sent, the TCP connection simply dies. The test then polls until the lock is observed released, and starts process B only afterward (sequential — "the second acquiring the lock afterwards," not a second racer), asserting it completes well inside a timeout and that `goose_db_version` ends at the full count. The property under test is Postgres's own guarantee — a session-scoped advisory lock is freed when its session ends, including on an unclean disconnect — not application code.
...
```text
process A killed at 2026-09-01T23:26:43.859501239-03:00 while holding the lock
lock observed released at 2026-09-01T23:26:43.863066612-03:00 (0.004s after kill)
process B started 2026-09-01T23:26:43.863066972-03:00, completed in 209.733184ms: cmd/migrate: applied 5 migration(s) (5 total)
final goose_db_version count: 5/5
--- PASS: TestIntegrationMigrateBootRaceSecondProcessRecoversAfterFirstIsKilled (4.03s)
```

Process A was killed within milliseconds of acquiring the lock — before it had applied any migration ("process A stdout before it was killed:" was empty) — because the window between lock acquisition and the first migration's DDL is itself narrow; a mid-*batch* crash (some, not all, of the 5 committed) is bounded by the same guarantee `TestIntegrationMigrateReleasesLockOnMidMigrationFailure` already covers for the graceful-failure path, since goose commits each migration file's transaction independently. What this test adds is specifically the *session-death* path: Postgres released the lock 4ms after the kill with zero application-level cleanup involved, and process B — started only after that release was observed, not racing A — completed in 210ms and left `goose_db_version` at the full count. No hang.
